...
import fileIcons from '../assets/fileIcons'; // Import file icons
...
function Home() {
  ...
  const onChange = (e) => {
    ...
    const fileMetadata = selectedFiles.map(file => ({
      ...
      icon: fileIcons[file.name.split('.').pop().toLowerCase()] || fileIcons['default'] // Set appropriate icon
    }));
            ...
            if (entry.isDirectory) {
              ...
              item.file(file => {
                files.push({
                  ...
                  lastModified: new Date(file.lastModified).toLocaleDateString(),
                  extension: file.name.split('.').pop().toLowerCase(),
                  icon: fileIcons[file.name.split('.').pop().toLowerCase()] || fileIcons['default']
                });
  ...
  const processEntry = (entry, path) => {
    if (entry.isDirectory) {
      const reader = entry.createReader();
      reader.readEntries(entries => {
        entries.forEach(subEntry => processEntry(subEntry, path + entry.name + '/'));
      });
    } else {
      entry.file(file => {
        setFiles(prevFiles => [
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    if (files.length === 0) {
      setErrorMessage('Please select at least one file or folder.');
      return;
    }
        ...
        <form onSubmit={onSubmit}>
          <div className="upload-area">
            <label htmlFor="file-upload" className="upload-label">
              ...
              />
              <span className="upload-text">Drag and drop a folder here or</span>
          ...
          {files.map((fileData, index) => (
            <div key={index} className="file-info">
              <div className="file-details">
                <img src={fileData.icon} alt="File Icon" className="file-icon" />
                <div className="file-metadata">
                  <div className="file-name">{fileData.name}</div>
                  <div className="file-size">{fileData.size} MB</div>
                </div>
                <div className="file-cancel" onClick={() => handleCancel(index)}>
                  ✖
                </div>
              </div>
              <table className="file-metadata-table">
                <tbody>
                  ...
                  <tr>
                    <td className="metadata-label">File Type:</td>
                  ...
                  <tr>
                    <td className="metadata-label">File Extension:</td>
                    <td className="metadata-value">{fileData.extension}</td>
                  </tr>
                </tbody>
              </table>
            </div>
